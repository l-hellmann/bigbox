...
- ✅ **GLTF import + node animation** — the stacked-cube box-people are replaced by (currently) Kenney's CC0 **rigged blocky characters** (`assets/characters/kennys/`, node-animated, no skin). `game/src/character.rs` owns loading + the animation layer, and is **deliberately art-agnostic** (the placeholders are WIP — see "Replacing the placeholder art" below). Per-model `AnimationGraph` built from the glb's clips **by name** (`build_char_graphs` reads `Gltf.named_animations` via the `CLIP_NAMES` map, so a model missing a clip degrades to idle), `attach_char_animation` resolves each spawned model's `AnimationPlayer`, and `drive_char_animation` binds the graph + cross-fades to the `CharModel.state` the reconcile sets each frame (idle↔walk↔sprint from ground speed, hold-gun / shoot for the player off the fire-cooldown rising edge, melee for enemies in range).
...
**Replacing the placeholder art (the Kenney characters are WIP).** The render / material / lighting / animation *systems* are art-agnostic; swapping in final character art touches only data, no system rewrites:
- **`character.rs` catalog** — `player_spec()` / `enemy_spec(id)` return a `CharSpec { model: &'static str, scale }` (model path + relative size). Edit the paths/scales; that's the archetype→model mapping.
- **`CLIP_NAMES`** — semantic state → glb clip *name*. Only touch it if the new models name their animation clips differently from the Kenney roster (`idle` / `walk` / `sprint` / `holding-both` / `holding-both-shoot` / `attack-melee-right` / `die`). Clips are bound **by name**, never by index.
- **Whole-set fit** — `CHAR_BASE_SCALE` (native→world size), `MODEL_FACING_OFFSET` (rest-pose forward vs +Z heading; `0` or `π`), `CHAR_NATIVE_HEIGHT` (health-bar anchor).
- **Skinned final art** — the placeholders are node-animated. A skinned/morph model bakes extra components (`SkinnedMesh`, `MorphWeights`, `MeshMorphWeights`, `NoFrustumCulling`) — already registered in `register_gltf_types`, so the reflection spawner won't panic. Lighting (`DirectionalLight` + `AmbientLight`) and world-geometry materials have zero character coupling.
